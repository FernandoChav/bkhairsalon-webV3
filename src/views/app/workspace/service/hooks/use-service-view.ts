...
interface UseServiceViewReturn {
    ...
    categoryMap: Map<string, { category: CategoryResponse; parent: CategoryResponse | null; level: number }>;
    // Handlers - Search
...
export const useServiceView = (): UseServiceViewReturn => {
    ...
    const [selectedParentCategory, setSelectedParentCategory] = useState<CategoryResponse | null>(null);
    const [isEditCategoryModalOpen, setIsEditCategoryModalOpen] = useState(false);
    const [selectedEditCategory, setSelectedEditCategory] = useState<CategoryResponse | null>(null);

    // Service states
    const [selectedService, setSelectedService] = useState<ServiceResponse | null>(null);
    const [isServiceSheetOpen, setIsServiceSheetOpen] = useState(false);
    const [isCreateServiceModalOpen, setIsCreateServiceModalOpen] = useState(false);
    const [selectedServiceCategory, setSelectedServiceCategory] = useState<CategoryResponse | null>(null);
    const [isUpdateServiceModalOpen, setIsUpdateServiceModalOpen] = useState(false);
    const [selectedUpdateService, setSelectedUpdateService] = useState<ServiceResponse | null>(null);
    ...
    const shouldCollapseCategory = useCallback((categoryId: string): boolean => {
    ...
    const handleCreateSubcategory = useCallback((parentCategory: CategoryResponse) => {
    ...
    const handleDragStart = useCallback((event: DragStartEvent) => {
        ...
        const elementType = activeId.startsWith('category-') ? 'category' : 'service';
    ...
    const handleDragEnd = useCallback((event: DragEndEvent) => {
        ...
        const activeType = activeId.startsWith('category-') ? 'category' : 'service';
